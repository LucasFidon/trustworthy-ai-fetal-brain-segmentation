...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_embedding_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   use_sliding_window: bool = False,
                   step_size: float = 0.5, patch_size: Tuple[int, ...] = None, regions_class_order: Tuple[int, ...] = None,
                   use_gaussian: bool = False, pad_border_mode: str = "constant",
                   pad_kwargs: dict = None, all_in_gpu: bool = False,
        ...
        assert self.conv_op == nn.Conv3d, "embedding prediction only supported for 3D models"
        if use_sliding_window:
            res = self._internal_predict_embedding_3D_3Dconv_tiled(x, step_size, do_mirroring, mirror_axes, patch_size,
                                                             regions_class_order, use_gaussian, pad_border_mode,
                                                             pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu,
                                                             verbose=verbose)
        else:
            res = self._internal_predict_embedding_3D_3Dconv(x, patch_size, do_mirroring, mirror_axes, regions_class_order,
                                                       pad_border_mode, pad_kwargs=pad_kwargs, verbose=verbose)
    ...
    def _internal_predict_embedding_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          patch_size: tuple, regions_class_order: tuple, use_gaussian: bool,
                                          pad_border_mode: str, pad_kwargs: dict, all_in_gpu: bool,
                                          verbose: bool) -> Tuple[np.ndarray, np.ndarray]:
        # better safe than sorry
        ...
        #TODO: number of features of the embedding is hard-coded
        num_fea = 30
        ...
        with torch.no_grad():
            ...
            if all_in_gpu:
                ...
                if verbose: print("initializing result array (on GPU)")
                aggregated_results = torch.zeros([num_fea] + list(data.shape[1:]), dtype=torch.half,
                                                 device=self.get_device())
                ...
                if verbose: print("initializing result_numsamples (on GPU)")
                aggregated_nb_of_predictions = torch.zeros([num_fea] + list(data.shape[1:]), dtype=torch.half,
                                                           device=self.get_device())
            else:
                if use_gaussian and num_tiles > 1:
                    add_for_nb_of_preds = self._gaussian_3d
                    ...
                    add_for_nb_of_preds = np.ones(data.shape[1:], dtype=np.float32)
                aggregated_results = np.zeros([num_fea] + list(data.shape[1:]), dtype=np.float32)
                aggregated_nb_of_predictions = np.zeros([num_fea] + list(data.shape[1:]), dtype=np.float32)
            ...
            for x in steps[0]:
                ...
                for y in steps[1]:
                    ...
                    for z in steps[2]:
                        ...
                        predicted_patch = self._internal_maybe_mirror_and_pred_embedding_3D(
                            data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring,
                            gaussian_importance_map)[0]
            ...
            # computing the class_probabilities by dividing the aggregated result with result_numsamples
            embedding_results = aggregated_results / aggregated_nb_of_predictions

            if all_in_gpu:
                if verbose: print("copying results to CPU")
                embedding_results = embedding_results.detach().cpu().numpy()

        print("prediction of embedding done")
    ...
    def _internal_predict_embedding_3D_3Dconv(self, x: np.ndarray, min_size: Tuple[int, ...], do_mirroring: bool,
                                    mirror_axes: tuple = (0, 1, 2), regions_class_order: tuple = None,
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        with torch.no_grad():
            data, slicer = pad_nd_image(x, min_size, pad_border_mode, pad_kwargs, True,
                                        self.input_shape_must_be_divisible_by)
            predicted_emb = self._internal_maybe_mirror_and_pred_embedding_3D(data[None], mirror_axes, do_mirroring,
                                                                              None)[0]
            slicer = tuple(
                [slice(0, predicted_emb.shape[i])
                 for i in range(len(predicted_emb.shape) - (len(slicer) - 1))] + slicer[1:])
            predicted_emb = predicted_emb[slicer]
            predicted_emb = predicted_emb.detach().cpu().numpy()
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        with torch.no_grad():
            ...
            if do_mirroring:
                ...
                print('Perform test time data augmentation with %d mirroring operations' % num_results)
            else:
                mirror_idx = 1
                num_results = 1
    ...
    def _internal_maybe_mirror_and_pred_embedding_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
                                           do_mirroring: bool = True,
                                           mult: np.ndarray or torch.tensor = None) -> torch.tensor:
        """
        Added by Lucas to compute and return the embedding map
        """
        assert len(x.shape) == 5, 'x must be (b, c, x, y, z)'
        # everything in here takes place on the GPU. If x and mult are not yet on GPU this will be taken care of here
        # we now return a cuda tensor! Not numpy array!
        with torch.no_grad():
            x = to_cuda(maybe_to_torch(x), gpu_id=self.get_device())
            #TODO: the number of features in the last layer is hard-coded
            num_fea = 30
            result_torch = torch.zeros([1, num_fea] + list(x.shape[2:]),
                                       dtype=torch.float).cuda(self.get_device(), non_blocking=True)
            ...
            for m in range(mirror_idx):
                if m == 0:
                    pred = self.embedding(self(x))
                    result_torch += 1 / num_results * pred

                if m == 1 and (2 in mirror_axes):
                    pred = self.embedding(self(torch.flip(x, (4, ))))
                    result_torch += 1 / num_results * torch.flip(pred, (4,))

                if m == 2 and (1 in mirror_axes):
                    pred = self.embedding(self(torch.flip(x, (3, ))))
                    result_torch += 1 / num_results * torch.flip(pred, (3,))

                if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                    pred = self.embedding(self(torch.flip(x, (4, 3))))
                    result_torch += 1 / num_results * torch.flip(pred, (4, 3))

                if m == 4 and (0 in mirror_axes):
                    pred = self.embedding(self(torch.flip(x, (2, ))))
                    result_torch += 1 / num_results * torch.flip(pred, (2,))

                if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                    pred = self.embedding(self(torch.flip(x, (4, 2))))
                    result_torch += 1 / num_results * torch.flip(pred, (4, 2))

                if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                    pred = self.embedding(self(torch.flip(x, (3, 2))))
                    result_torch += 1 / num_results * torch.flip(pred, (3, 2))

                if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                    pred = self.embedding(self(torch.flip(x, (4, 3, 2))))
                    result_torch += 1 / num_results * torch.flip(pred, (4, 3, 2))
