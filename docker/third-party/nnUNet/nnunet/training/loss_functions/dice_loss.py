...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    tp = net_output * y_onehot  # b,c,x,y,z
...
class SoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1., reduction='mean'):
        """
        """
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if self.batch_dice:
            # Compute one dice loss for the full batch
            # instead of one for each sample in the batch.
            # This is used by default for the 2d model.
            axes = [0] + list(range(2, len(shp_x)))
        else:
            axes = list(range(2, len(shp_x)))
        ...
        # Lucas: better to remove the smooth term from the numerator
        ...
        if self.batch_dice:
            # no more batch dimension in that case
            return -dc
...
class SoftDiceLossSquared(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        intersect = sum_tensor(intersect, axes, False)
...
class DL_and_Focal_loss(nn.Module):
    def __init__(self, dl_kwargs, focal_kwargs, aggregate="sum",
                 weight_focal=1, weight_dl=1, reduction='mean'):
        super(DL_and_Focal_loss, self).__init__()
        self.weight_dl = weight_dl
        self.weight_focal = weight_focal
        self.aggregate = aggregate
        self.focal = FocalLoss(**focal_kwargs)
        self.dl = SoftDiceLoss(apply_nonlin=softmax_helper, **dl_kwargs)
        self.focal.reduction = reduction
        self.dl.reduction = reduction

    def forward(self, net_output, target):
        dl_loss = self.dl(net_output, target) if self.weight_dl != 0 else 0
        focal_loss = self.focal(net_output, target) if self.weight_focal != 0 else 0
        if self.aggregate == "sum":
            result = self.weight_focal * focal_loss + self.weight_dl * dl_loss
        else:
            raise NotImplementedError("nah son") # reserved for other stuff (later)
        return result
...
class GWDL_and_Focal_loss(nn.Module):
    def __init__(self, gwdl_kwargs, focal_kwargs, aggregate="sum",
                 weight_focal=1, weight_gwdl=1, reduction='mean'):
        super(GWDL_and_Focal_loss, self).__init__()
        self.weight_gwdl = weight_gwdl
        self.weight_focal = weight_focal
        self.aggregate = aggregate
        self.focal = FocalLoss(**focal_kwargs)
    ...
    def forward(self, net_output, target):
        ...
        focal_loss = self.focal(net_output, target) if self.weight_focal != 0 else 0
        if self.aggregate == "sum":
            result = self.weight_focal * focal_loss + self.weight_gwdl * gwdl_loss
        else:
            raise NotImplementedError("nah son") # reserved for other stuff (later)
        return result
    ...
    @property
    def reduction(self):
        return self.focal.reduction  # should be the same as self.dc.reduction
