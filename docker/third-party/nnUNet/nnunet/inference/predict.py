...
def predict_cases_embedding(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
                  num_threads_nifti_save, segs_from_prev_stage=None, do_tta=True, fp16=None, overwrite_existing=False,
                  all_in_gpu=False, step_size=0.5, force_separate_z=None, interp_order=3, interp_order_z=0,
                  checkpoint_name="model_final_checkpoint"):
    ...
    # try to plot the network architecture
    ...
    for preprocessed in preprocessing:
        ...
        for p in params:
            trainer.load_checkpoint_ram(p, False)
            embedding.append(trainer.predict_preprocessed_data_return_embedding(
               d, do_tta, trainer.data_aug_params['mirror_axes'], True, step_size=step_size, use_gaussian=True,
        ...
        if transpose_forward is not None:
            transpose_backward = trainer.plans.get('transpose_backward')
            embedding_mean = embedding_mean.transpose([0] + [i + 1 for i in transpose_backward])

        print('We have not resampled if needed... ok for BraTS but might fail in the future...')
        print('Save ', output_filename[:-7] + "_embedding.npy")
        # Compression did not work...
        # np.savez_compressed(output_filename[:-7] + "_embedding.npz", embedding=embedding_mean.astype(np.float16))
        save_pickle(dct, output_filename[:-7] + "_embedding.pkl")
        np.save(output_filename[:-7] + "_embedding.npy", embedding_mean)
...
def predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
                        ...
                        checkpoint_name="model_final_checkpoint", save_npz_embedding=False):
    """
    ...
    if mode == "normal":
        ...
        if save_npz_embedding:
            # Lucas's stuff
            print('Predict and save the embedding maps (.npz)')
            return predict_cases_embedding(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts],
                                 folds, num_threads_preprocessing, num_threads_nifti_save, lowres_segmentations,
                                 ...
                                 interp_order_z=interp_order_z, checkpoint_name=checkpoint_name)
        else:
            # Do classic nnUNet stuff
            return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                                 save_npz, num_threads_preprocessing, num_threads_nifti_save, lowres_segmentations,
                                 tta, fp16=fp16, overwrite_existing=overwrite_existing, all_in_gpu=all_in_gpu,
                                 step_size=step_size, force_separate_z=force_separate_z, interp_order=interp_order,
                                 interp_order_z=interp_order_z, checkpoint_name=checkpoint_name)
    elif mode == "fast":
        ...
        assert save_npz_embedding is False
