...
def copy_BraTS_segmentation_and_convert_labels(in_file, out_file):
    ...
    seg_new[img_npy == 1] = 2  # Non-enhancing Tumor (NET)
...
if __name__ == "__main__":
    ...
    task_name = "Task044_BraTS2020"
    downloaded_data_dir = os.path.join(DATA_FOLDER, "MICCAI_BraTS2020_TrainingData")
    ...
    # Validation data
    downloaded_data_dir = os.path.join(DATA_FOLDER, "MICCAI_BraTS2020_ValidationData")
    ...
    # Testing data
    downloaded_test_data_dir = os.path.join(DATA_FOLDER, "MICCAI_BraTS2020_TestingData")
    if os.path.exists(downloaded_test_data_dir):
        for p in subdirs(downloaded_test_data_dir, join=False):
            patdir = join(downloaded_test_data_dir, p)
            patient_name = p
            t1 = join(patdir, p + "_t1.nii.gz")
            t1c = join(patdir, p + "_t1ce.nii.gz")
            t2 = join(patdir, p + "_t2.nii.gz")
